...
- If a command fails due to sandbox, retry with:
  - `allow_network: true` if network is blocked (requires user approval).
  - `unsandboxed: true` if filesystem is read-only (requires user approval).
